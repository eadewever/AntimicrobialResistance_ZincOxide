#This script aims at displaying the phenotypic resistance profiles of the isolates using heatmaps
...
toxins <- colnames(AST_data_filtered)[21:22] #Only STa and LTa as no Stx2e positive isolates (Stx2e thus not displayed)
fimbriae <- colnames(AST_data_filtered)[24:25] #Only F4 and F18 were detected in our isolates so we are not displaying the other ones
visits <- unique(AST_data_filtered$Visit)


#Adding a column to AST_data_filtered with one unique ID per isolate (Farm not indicated for confidentiality reasons)
...
#Creating separate tables, for the antibiotics, the toxins and the fimbriae
...
#Renaming the rows the annotations with IsolateID (so that they match with data_antibiotics_normalised)
...
#Defining colours for the antibiotics heatmap
#If normalised value > 1 = susceptible isolate = shades of green vs < 1 = resistant = shades of pink/red

#We want the max value of the whole normalised dataframe (to adjust the scale properly)
max_val_palette <- max(data_antibiotics_normalised, na.rm = TRUE)
  
#We want 1000 different colors with a step of 1 in the legend, i.e. 1000 from 0 to 1, 1000 from 1 to 2, etc.
#The way the palette works, all values ≤ 1 will be displayed in shades of red
n_low <- 1000
...
breaks <- c(
  seq(0, 1, length.out = n_low), #to be coloured in red 
  seq(1 + .Machine$double.eps, max_val_palette, length.out = n_high) #to be coloured in green
)
...
#Creating the heatmap with toxins and visit
filename <- "Graphs/AST_heatmap_ToxVisit.png"
print(paste("Saving:", filename))
png(filename, width = 3500, height = 5500, res = 400)

pheatmap(data_antibiotics_normalised,
         cluster_rows = TRUE, #clustering the isolates
         cluster_cols = TRUE, #clustering the antibiotics
         ...
         fontsize_col = 12,
         fontsize = 10,
         #main = "Isolates - Antimicrobial Resistance Profiles",
         legend_breaks = c(0, 1, 2),  #Antibiotics legend breaks
...
#Creating the heatmap with toxins, fimbriae and visit
filename <- "Graphs/AST_heatmap_ToxFimVisit.png"
print(paste("Saving:", filename))
png(filename, width = 3500, height = 5500, res = 400)
...
#Creating a heatmap split by cluster
filename <- "Graphs/AST_heatmap_ToxFimVisit_split.png"
print(paste("Saving:", filename))
png(filename, width = 3500, height = 5500, res = 400)
...
pheatmap(data_antibiotics_normalised,
         ...
         clustering_distance_rows = 'euclidean',
         clustering_method = "ward.D2",
         cutree_rows = 20, #cutting to get 20 clusters
...
#Creating one heatmap per sample to see the isolate diversity within each sample (indicating toxins, fimbriae and visit)
#Retrieving the ID of the samples (e.g. V1_W01 from the isolates ID, i.e. V1_W01_P1)
samples_ID <- sapply(rownames(data_antibiotics_normalised), function(x) {
  paste(strsplit(x, "_")[[1]][1:2], collapse = "_")
})

#Creating lists of all the isolatesID per sample
samples_isolateslist <- split(rownames(data_antibiotics_normalised), samples_ID)

#Separating the whole matrix into sub-matrices with one by sample
samples_submatrices <- lapply(samples_isolateslist,
                       function(rows) data_antibiotics_normalised[rows, , drop = FALSE])


#Generating one heatmap per sample
for (sample_name in names(samples_submatrices)) {
  
  # Creating filename and printing it (in case of error)
  filename <- paste0("Graphs/", sample_name, "_heatmap.png")
  print(paste("Saving:", filename))
  
  # Opening the PNG device to save the plot
  png(filename, width = 5000, height = 3000, res = 400)
  
  #Creating annotation colors for normalised zones
  max_val_palette <- max(samples_submatrices[[sample_name]], na.rm = TRUE)
  ...
  pheatmap(samples_submatrices[[sample_name]],
           cluster_rows = TRUE,  # Clustering the isolates
           cluster_cols = TRUE,  # Clustering the antibiotics
           clustering_distance_cols = 'euclidean',
           clustering_distance_rows = 'euclidean',
           ...
           fontsize_row = 8,
           fontsize_col = 15,
           main = paste(sample_name, "heatmap"),
           legend_breaks = c(0, 1, 2),  # Antibiotics legend breaks
           legend_labels = c("Resistant (≤ 1)", "1.0", "Susceptible (> 1)"),
           angle_col = 45,  # Rotating antibiotics and toxins names
           legend = FALSE, #In order to not display the legend
           annotation_legend = FALSE) #In order to not display the annotation legend
  
  # Closing the device (and saving the plot)
  dev.off()
  
  # For V4_W04, V4_W05 and V4_W07, isolation done twice -> creating heatmap without cluster to compare to heatmap of additional picks (cf. file Additional_picks_heatmap)
  if (sample_name %in% c("V4_W04", "V4_W05", "V4_W07")) {
    filename2 <- paste0("Graphs/", sample_name, "_heatmap_nocluster.png")
    print(paste("Saving no-cluster heatmap:", filename2))
    
    png(filename2, width = 4500, height = 2500, res = 400)
    ...
    pheatmap(samples_submatrices[[sample_name]],
             ...
             fontsize_col = 15,
             fontsize = 10,
             main = paste(sample_name, "heatmap (no clustering)"),
